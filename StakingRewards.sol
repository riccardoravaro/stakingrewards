...
contract StakingRewards is Owned {
    ...
    function getReward() public updateReward(msg.sender) {
        uint256 reward = _calculateUnlocked(rewards[msg.sender]) - rewardsPaid[msg.sender];
        if (reward > 0) {
    ...
    // TODO recalculate reward rate in proper way
        ...
        onlyOwner
    {
        if (tokenAddress == address(stakingToken)) {
            ...
            // total token balance of the contract
    ...
    modifier onlyBeforePeriodFinish() {
        require(block.timestamp < periodFinish, "412: period is ended");
        _;
    }
